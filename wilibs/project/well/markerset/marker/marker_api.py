...
def createMarker_RAW(token, markerSetId, markerTemplateId, Depth):
    url = genUrlWithWiId(ROOT_API + '/project/well/marker-set/marker/new',
                         {'idMarkerSet': markerSetId, 'idMarkerTemplate': markerTemplateId, 'depth': Depth}, token)
    r = requests.post(url, json={'idMarkerSet': markerSetId, 'idMarkerTemplate': markerTemplateId, 'depth': Depth},
                      headers=tokenHeader(token), verify=False)
    return r.json()


def deleteMarker_RAW(token, markerId):
    url = genUrlWithWiId(ROOT_API + '/project/well/marker-set/marker/delete', {'idMarker': markerId}, token)
...
def getListMarker_RAW(token, markerSetId):
    url = genUrlWithWiId(ROOT_API + '/project/well/marker-set/marker/list', {'idMarkerSet': markerSetId}, token)
...
def getMarkerInfo_RAW(token, markerId):
    url = genUrlWithWiId(ROOT_API + '/project/well/marker-set/marker/info', {'idMarker': markerId}, token)
